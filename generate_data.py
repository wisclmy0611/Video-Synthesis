...
output_path_format = 'dataset/clip_{:03d}/window_{:01d}'
...
def main():
    ...
    clip_starting_points = list(range(1, num_files + 1, num_frames_per_clip))
    ...
    for clip, starting_point in enumerate(clip_starting_points):
        ...
        for window in range(num_rows_per_clip * num_cols_per_clip):
            ensure_path(output_path_format.format(clip, window))
        ...
        for frame in range(num_frames_per_clip):
            ...
            for row in range(num_rows_per_clip):
                for col in range(num_cols_per_clip):
                    ...
                    cropped.save(output_file_format.format(clip, window, frame))
